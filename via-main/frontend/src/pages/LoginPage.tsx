...
export function LoginPage({ onLoginSuccess, onBack, targetScreen }: LoginPageProps) {
  ...
  const loginCopy = useMemo(() => {
    if (targetScreen === "finance") {
      return {
        title: "Finans Merkezi Girişi",
        description:
          "Tahsilat, gider, cari hesap, kasa/banka, devir ve dönem kapanışı işlemleri için güvenli giriş alanı.",
      };
    }

    return {
      title: "Operasyon Merkezi Girişi",
      description:
        "Teklif, anlaşma, etkinlik dosyaları, hizmet kataloğu ve saha operasyonları için güvenli giriş alanı.",
    };
  }, [targetScreen]);
  ...
  return (
    <main className="min-h-screen bg-slate-950 px-5 py-6 text-white">
      <div className="mx-auto flex min-h-[calc(100vh-3rem)] w-full max-w-md flex-col justify-center">
        <button
          onClick={onBack}
          className="mb-6 w-fit rounded-full border border-white/10 bg-white/10 px-4 py-2 text-sm text-slate-200 transition hover:bg-white/15"
        >
          ← Ana ekrana dön
        </button>
        ...
        <section className="rounded-[2rem] border border-white/10 bg-white/10 p-5 shadow-2xl shadow-black/30 backdrop-blur">
          <div className="rounded-[1.5rem] bg-white p-5 text-slate-950">
            ...
            />
            <h1 className="mt-3 text-3xl font-black">{loginCopy.title}</h1>
