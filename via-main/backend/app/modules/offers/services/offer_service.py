...
def create_offer(db: Session, payload: OfferCreate):
    ...
    # Teklif numarasını teklifin GERÇEK id'sinden türet. id birincil anahtar
    # olduğu için benzersizdir; böylece offer_no (ve ondan türeyen event_code)
    # çakışması imkânsız hâle gelir. Eski max(id)+1 yöntemi, silme/iptal veya
    # ardışık senaryolarda aynı numarayı üretip UNIQUE çakışmasına yol açıyordu.
    offer = offer_repository.create_offer(db=db, data=data)
    offer.offer_no = f"VIA-TEK-{offer.created_at.year if offer.created_at else date.today().year}-{offer.id:05d}"
    db.commit()
    db.refresh(offer)
